...
SQL_CREATE = "CREATE TABLE IF NOT EXISTS transactions (" \
             "transaction_id INTEGER PRIMARY KEY AUTOINCREMENT,timestamp TEXT, txn_id TEXT,myaddress TEXT, recipient TEXT, amount INTEGER, signature TEXT, " \
             "public_key_hashed TEXT,operation TEXT, openfield TEXT, fee INTEGER)"

# Selects all tx from mempool - list fields so we don't send mergedts and keep compatibility
SQL_SELECT_ALL_TXS = 'SELECT timestamp, myaddress, recipient, amount, signature, operation, openfield,fee FROM transactions'
...
class Mempools:
    ...
    def __init__(self, config=None, db_lock=None, testnet=False):
        try:
            ...
            #     print("Regtest mode, ram mempool")
            #     self.ram = True
            ...
            if not self.testnet:
                self.mempool_ram_file = "mempool.db"
            else:
                print("Starting mempool in testnet mode")
                self.mempool_ram_file = "mempool.db"

            self.check()

        except Exception as e:
            print("Error creating mempool: {}".format(e))
            raise
    ...
    def check(self):
        ...
        with self.lock:
            if self.ram:
                ...
                self.db.commit()
                print("Status: In memory mempool file created")
            else:
                ...
                if len(res) != 9:
                    ...
                    self.commit()
                    print("Status: Recreated mempool file")

    def execute(self,file, sql, param=None, write=False):
        """
        ...
        if write:
            try:
                ...
                print("Database query: {} {}".format(cursor, sql))
                print("Database retry reason: {}".format(e))
                time.sleep(0.1)
                return False

    def commit(self,file,sql,write=False):
        """
        Safe commit
        :return:
        """
    ...
    def Fetchone(self,file,sql, param=None, write=False):
        """
        ...
        if write:
            try:
                ...
                time.sleep(0.1)
                return False
    def Fetchall(self,file,sql, param=None, write=False):
        """
        ...
        if write:
            try:
                ...
                print("Database query:  {}".format(sql))
                print("Database retry reason: {}".format(e))
                time.sleep(0.1)
                return False
