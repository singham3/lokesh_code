...
def encrypt_fn(destroy_this):
    ...
    if password == password_conf:
        ...
        try:
            ...
            keyring.key, keyring.public_key_readable, keyring.private_key_readable, keyring.encrypted, keyring.unlocked, keyring.public_key_hashed, keyring.myaddress, keyring.keyfile = essentials.keys_load_new(keyring.keyfile)
...
def send_confirm(sender,amount,recipient, operation,openfield):
    ...
    # cryptopia check
    if recipient == keyring.myaddress and len(openfield) not in [1, 20]:
    ...
    balance,recv,send=UTXO.balance(keyring.myaddress)
    if balance<Decimal(amount) + Decimal(fee):
        messagebox.showinfo("Your, balance is low please add Cowrie")
    else:
...
def refresh():
    if not keyring.encrypted:
        messagebox.showwarning("Error", "Encrypt Your Wallet")
        return
    ...
    for i in range(len(transfered_txn)):
        txn = (int(transfered_txn[i][0]),transfered_txn[i][1],transfered_txn[i][2],transfered_txn[i][3],transfered_txn[i][4],float(transfered_txn[i][5]),transfered_txn[i][6],transfered_txn[i][7],transfered_txn[i][8],float(transfered_txn[i][9]),transfered_txn[i][10],transfered_txn[i][11],transfered_txn[i][12])
    ...
    for i in range(len(transfered_txn1)):
        txn = (int(transfered_txn1[i][0]), transfered_txn1[i][1], transfered_txn1[i][2], transfered_txn1[i][3],
               transfered_txn1[i][4], float(transfered_txn1[i][5]) - float(transfered_txn1[i][11]),
               transfered_txn1[i][6], transfered_txn1[i][7], transfered_txn1[i][8], transfered_txn1[i][9],
               transfered_txn1[i][10], float(transfered_txn1[i][11]))

        c = Mempool.Fetchall('static/ledger.db', "SELECT * FROM transfered WHERE signature=?", (transfered_txn1[i][6],), True)
        if c:
            pass
        else:
            mp1 = Mempool.execute('static/ledger.db',"INSERT INTO transfered(block_height, timestamp,Nones, sender, recipient, amount, signature, public_key, block_hash, operation, openfield,fee) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",txn,True)
    ...
    debit_var.set("Send Total: {:.8f} CWR".format(Decimal(send)))
...
def Send_request_money(address,amount,Operation):
    ...
    if not address_validate(address):
        ...
        if verifier.verify(h, signature):
            ...
            #print(str(tx_timestamp), str(keyring.myaddress), str(recipient), '%.8f' % float(amount),str(signature_enc), str(operation), '%.8f' % float(fee), str(openfield))
            submit = str(tx_timestamp), str(keyring.myaddress), '%.8f' % float(amount), str(signature_enc), str(keyring.public_key_hashed.decode("utf-8")), str(operation)
            while True:
                try:
                    ...
                    print(reply)
                    if reply == "Success":
                        messagebox.showinfo("OK", "Money added succesfully. This process will take few time, please wait...")
                    else:
...
def Addmoney():
    ...
    add_button = Button(addbutton, width=60,text="Add Money", command=lambda: request_process(keyring.myaddress,str(amount.get()).strip(),str(Operation.get()).strip(),top))
...
if __name__ == "__main__":
    ...
    # supportbutton
    dev_support = Button(frame_support, text="Collect Info for Support",command=Addmoney, font=("Tahoma", 7))
